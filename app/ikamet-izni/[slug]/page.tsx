...
import { getResidenceService, residenceServices } from "@/lib/residenceServices";
import { site } from "@/lib/site";

export function generateStaticParams() {
  return residenceServices.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const s = getResidenceService(slug);
  if (!s) return {};
  return {
    title: s.title,
    description: s.metaDescription,
    alternates: { canonical: `/ikamet-izni/${s.slug}` },
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = residenceServices.filter((x) => x.slug !== s.slug).slice(0, 3);
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-navy-900 pb-16 pt-36 text-white sm:pt-40">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <nav className="mb-5 flex flex-wrap items-center gap-2 text-sm text-navy-300">
            ...
            <Link href="/ikamet-izni" className="hover:text-gold-300">İkamet İzni</Link>
            <span className="text-gold-500">/</span>
      ...
      <section className="border-t border-navy-100 py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="mb-6 font-display text-xl font-semibold text-navy-900">İlgili İkamet İzni Konuları</h2>
          <div className="grid gap-5 sm:grid-cols-3">
            {related.map((r) => (
              <Link
                key={r.slug}
                href={`/ikamet-izni/${r.slug}`}
